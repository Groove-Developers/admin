   ...
   if(isset($_POST["submit"])){
    ...
    if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $target_file)) {
      ...
      " has been uploaded";
        ...
        $query = "UPDATE domain Set status='$status', domain_url='$domain_url', domain_tagline='$domain_tagline', domain_desc='$domain_desc', minimum_pay='$minimum_pay', maximum_pay='$maximum_pay', username='$username', reg_date='$reg_date' Where id='".$id."'";
    ...
    while($row = $result->fetch_assoc()) {
    	...
    	?>
								...
								<div class="form-group">
											<label>Graphic url</label>
												<input type="url" class="form-control1" id="domain_url" placeholder="" name="domain_url" value=<?php echo $row['domain_url']; ?>>
										</div>
										<div class="form-group">
											<label>Tagline</label>
												<input type="text" class="form-control1" id="domain_tagline" placeholder="e.g Media Site" name="domain_tagline" value=<?php echo $row['domain_tagline']; ?>>
										</div>
										<div class="form-group">
											<label >Status</label>
        ...
        <option value="Purchased">Purchased</option>
										...
										<div class="form-group">
											...
											<textarea name="domain_desc" id="domain_desc" cols="50" rows="4" class="form-control"  ><?php echo $row['domain_desc']; ?></textarea></div>
										</div>
										
										<div class="form-group">
										<label>Drop the Domain credentials here</label>
										<input type="file" id="uploaded_file" name="uploaded_file" value=<?php echo $row['domain_file']; ?>>
